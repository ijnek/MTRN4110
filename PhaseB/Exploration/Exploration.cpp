    ...
    int y = mapY;
        ...
        if (startingPose != Unknown)
        {
            switch(startingPose)
            {
                ...
                {
                    std::string hAndVWall = explorationMaze.encodeHAndVWall(
                ...
                case RightTopFacingDown:
                {
                    ...
                    std::string hAndVWall = explorationMaze.encodeHAndVWall(EXPLORATION_STARTING_X, EXPLORATION_STARTING_Y, EXPLORATION_STARTING_FACING_DIRECTION,
        ...
        else
        {
            ...
            if (!possibleMoveDirections.empty())
            {
                ...
                for (unsigned i = 0; i < possibleMoveDirections.size(); ++i)
                {
                    ...
                    if (goalX > 0)
                    {
                        val += std::abs(nextX - goalX);
                    }
                    if (goalY > 0)
                    {
                        val += std::abs(nextY - goalY);
                    }
        ...
        if (!blackboard.reachedGoal)
        {
            ...
            blackboard.x = mapX;
            blackboard.y = mapY;
            blackboard.facingDirection = facingDirection;
        }
        ...
        // std::string nextWayPoint = "nextWayPoint: " + std::string(nextWayPointX) + ", " + std::string(nextWayPointY);
